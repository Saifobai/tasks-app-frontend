...
const TaskList = () => {
    ...
    const handleInputChange = (e) => {
        ...
        setFormDatat({ ...formData, [name]: value });
    };
    ...
    const createTask = async (e) => {
        ...
        try {
            await axios.post(`${URL}/api/tasks/singletask`, formData);
            toast.success("Task successfully added");
            setFormDatat({ ...formData, name: "" });
            getTasks()
        } catch (error) {
            toast.error(error.message);
        }
    ...
    const deleteTask = async (id) => {
        try {
            await axios.delete(`${URL}/api/tasks/deletetask/${id}`);
            getTasks();
    ...
    const getSingleTask = async (task) => {
        setFormDatat({ name: task.name, completed: false })
        setTaskID(task._id)
        setIsEditing(true)
    ...
    return (
        <div>
            ...
            {!isLoading && tasks.length === 0 ? (
                <p className="--py">No task added please add one</p>
            ) : (
                <>
